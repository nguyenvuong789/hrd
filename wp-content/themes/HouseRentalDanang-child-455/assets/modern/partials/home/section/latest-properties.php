...
if ( ! empty( $sorty_by ) ) {
	if ( 'low-to-high' === $sorty_by ) {
		...
		$home_args['order']    = 'ASC';
	} else if ( 'high-to-low' === $sorty_by ) {
		$home_args['orderby']  = 'meta_value_num';
		$home_args['meta_key'] = 'REAL_HOMES_property_price';
		$home_args['order']    = 'DESC';
	} else if ( 'random' === $sorty_by ) {
...
?>
<section id="home-properties-section" class="rh_section rh_section--props_padding rh_latest-properties <?php echo esc_attr( $ajax_class ) . ' ' . esc_attr( $border_class ); ?>">
        ...
        <div id="home-properties-section-wrapper" class="wrapper-section-contents">
            <div id="home-properties-section-inner">
				...
				if ( $home_properties_query->have_posts() ) {
					...
					<?php
					$view_more_url = get_post_meta( get_the_ID(), 'inspiry_home_properties_link_view_more', true );
...
<?php
// Render the independently configured apartment feed after the native feed.
$show_second = get_post_meta( get_the_ID(), 'theme_show_home_properties_2', true );
if ( 'false' !== $show_second ) {
	get_template_part( 'assets/modern/partials/home/section/latest-properties-second' );
}
